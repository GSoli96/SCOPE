...
import sys
import pandas as pd
from configuration import BrowserDriverConfigurations
...
class Twitter_Scraper:
    def __init__(
        ...
        print("[X (Twitter)] Initializing Twitter Scraper...")
    ...
    def _config_scraper(
        ...
        self.tweet_cards = []
        self.max_tweets = max_tweets
        self.progress = Progress(current=0, total=max_tweets)
        self.scraper_details = {
            "type": None,
    ...
    def _get_driver(self):
        ...
        print("[X (Twitter)] Setup WebDriver...")
        ...
        try:
            if platform.system() == "Darwin":  # macOS
                base_path = os.path.expanduser("~/Library/Application Support/Google/Chrome/Profile 4'")
            elif platform.system() == "Windows":  # Windows
                base_path = os.path.join(os.getenv("LOCALAPPDATA"), "Google", "Chrome", "User Data", "Profile 12")
            elif platform.system() == "Linux":  # Linux
                base_path = os.path.expanduser("~/.config/google-chrome/Profile 1")
            else:
        ...
        print("[X (Twitter)] Platform: ", platform.system(), "Profile: ", base_path)
        ...
        try:
            print("[X (Twitter)] Initializing ChromeDriver...")
            driver = webdriver.Chrome(
                options=browser_option,
            )

            print("[X (Twitter)] WebDriver Setup Complete")
            return driver
        except WebDriverException:
            try:
                print("[X (Twitter)] Downloading ChromeDriver...")
                ...
                print(f"[X (Twitter)] Error setting up WebDriver: {e}")
                sys.exit(1)
        pass

    def login(self):
        print("[X (Twitter)] Logging in to Twitter...")
        ...
        try:
            ...
            if auth_token is None:
                raise ValueError(
                    """ [X (Twitter)] This may be due to the following:
...
"""
            ...
            print(f"[X (Twitter)] Login Failed: {e}")
            sys.exit(1)
...
- Twitter is experiencing unusual activity"""
    ...
    def go_to_search(self):
        if self.scraper_details["query"] is None or self.scraper_details["query"] == "":
            print("[X (Twitter)] Query is not set.")
            sys.exit(1)
        else:
    ...
    def scrape_tweets(
        ...
        if self.scraper_details["type"] == "Username":
            print(
                "[X (Twitter)] Scraping Tweets from @{}...".format(self.scraper_details["username"])
            )
        elif self.scraper_details["type"] == "Hashtag":
            print(
                "v[X (Twitter)] Scraping {} Tweets from #{}...".format(
                    self.scraper_details["tab"], self.scraper_details["hashtag"]
                )
            )
        elif self.scraper_details["type"] == "Query":
            print(
                "[X (Twitter)] Scraping {} Tweets from {} search...".format(
                    self.scraper_details["tab"], self.scraper_details["query"]
                )
            )
        elif self.scraper_details["type"] == "Home":
            print("[X (Twitter)] Scraping Tweets from Home...")
        
        self.progress.print_progress(current=0)
            ...
            try:
                            ...
                            if tweet:
                                if not tweet.error and tweet.tweet is not None:
                                    if not tweet.is_ad:
                                        ...
                                        added_tweets += 1
                                        self.progress.print_progress(current=len(self.data))
                ...
                if added_tweets == 0:
                    if empty_count >= 5:
                        if refresh_count >= 3:
                            print()
                            print("[X (Twitter)] No more tweets to scrape")
                            break
                        refresh_count += 1
                    empty_count += 1
                    sleep(1)
                ...
                print("\n")
                print("[X (Twitter)] Keyboard Interrupt")
                self.interrupted = True
                break
            except Exception as e:
                print("\n")
                print(f"[X (Twitter)] Error scraping tweets: {e}")
                break

        print("")

        if len(self.data) >= self.max_tweets:
            print(f"[X (Twitter)] Scraping {scrape_query} Complete")
        else:
            print(f"[X (Twitter)] Scraping {scrape_query} Incomplete")

        print("[X (Twitter)] {} out of {}\n".format(len(self.data), self.max_tweets))
        return self.data
        pass

    def save_to_csv(self, full_name, path_to_csv):
        print("[X (Twitter)] Saving Tweets to CSV...")
        # now = datetime.now()
        folder_path = path_to_csv

        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
            print(" [X (Twitter)] Created Folder: {}".format(folder_path))
        ...
        file_path = os.path.join(folder_path,full_name+'_twitter.csv')
        pd.set_option("display.max_colwidth", None)
        df.to_csv(file_path, index=False, encoding="utf-8", sep=';')

        print("[X (Twitter)] CSV Saved: {}".format(file_path))
...
class Progress:
    ...
    def print_progress(self, current) -> None:
        ...
        sys.stdout.write(
            "\n\rScraping in Progress: [{:<40}] {:.2%} {} of {}".format(
